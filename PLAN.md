...
### Спринт 1 (2 недели) — «Рабочий MVP» ✅ закрыт
...
| **SQLite (`node:sqlite`)** | Хранение данных | Надежное локальное хранилище, совместимо с ростом MVP |

### Что добавить поэтапно

| Этап | Технология | Зачем |
|------|------------|-------|
| Сейчас | Node + TypeScript + SQLite | Базовый production-ready контур |
| Спринт 2 | Web mini-app admin | Ускорение операций и контроля качества |
...
handler получает ctx (контекст: user, message, chat)
    ↓
tryJoinContest() проверяет условия и обновляет запись конкурса
    ↓
repository.update(...) сохраняет данные в SQLite
    ↓
...
```
data/contests.db
contests(id TEXT PRIMARY KEY, data TEXT, updated_at TEXT)
```

**Почему SQLite сейчас:** Production-ready локальное хранилище, атомарные операции и простой деплой без внешней БД.
